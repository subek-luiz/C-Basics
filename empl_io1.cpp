...
void employee::read()
{
    ...
    while(true)     // read type of next employee
    {
        ...
        // Read only the data members into the correct part of the object
     //   infile.read(reinterpret_cast<char*>(arrap[n]) + sizeof(employee), size - sizeof(employee));


        // Read the employee base class portion first
        infile.read(reinterpret_cast<char*>(arrap[n]), sizeof(employee));
        // Read the derived class portion if any
        infile.read(reinterpret_cast<char*>(arrap[n]) + sizeof(employee), size - sizeof(employee));
